...
public:
    GameServer(int port, std::string name) : port(port), game_name(name) {
        ...
        // Where we store the game_history
        ...
        std::cout << "======================================\n";
        std::cout << "Game Server Started: " << game_name << "\n"; // Show name
        std::cout << "======================================\n";
        std::cout << "Port: " << port << "\n";
    ...
    void receive_messages() {
        ...
        for (int id : disconnected) {
            ...
            report_to_python(this->game_name, p->get_name(), p->get_score(), p->get_playtime());
            close(p->get_socket());
            delete p;
            players.erase(id);
        }
...
int main(int argc, char* argv[]) {
    int port = 8080;
    std::string game_name = "UnknownGame"; // Default name
