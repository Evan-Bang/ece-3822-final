...
class TimeTravel:
    ...
    def record_state(self, player_x, player_y):
        ...
        if self.frames_since_last_record >= self.sample_rate:
            game_state = GameState(player_x,player_y,self.frame_counter)
            self.history.push(game_state)
            if self.history._size > self.max_history:
                temp_stack = Stack()
