...
class Level:
    def __init__(self, player_name, save_slot, server_host='localhost', server_port=8080, serializer='text', clock=None):
        ...
        # Store save data for player creation
        self.save = save_slot
        ...
        # Network update throttling (reduce CPU/network overhead)
        self.network_update_interval_ms = 100  # send/get updates at 10Hz instead of 60Hz
        self.last_network_update_ms = pygame.time.get_ticks()
    ...
    def _load_gid_map(self, tmx_path):
        ...
        Each tileset listed in the TMX has a firstgid and references a .tsx
        file.  The .tsx file names the image source.  We ignore the source
        path (which is usually an absolute path on the designer's machine) and
        instead look for an image with the same filename in the same directory
        as the TMX file.  Tiles are sliced at the tilewidth/tileheight
        declared in each TSX.
    ...
    def create_map(self):
        ...
        Falls back to the WORLD_MAP string grid for boundary tiles if
            ...
            try:
                ...
                for (row, col), tile_id in entries:
                    ...
                    if surf is not None:
                        ...
                        print(f"[WARNING] Missing GID: {tile_id}")
    ...
    def update_network(self):
        ...
        if now_ms - self.last_network_update_ms < self.network_update_interval_ms:
            return

        self.last_network_update_ms = now_ms
    ...
    def draw_status(self):
        ...
        # For static text, pre-render only once so this method is lighter
        if not hasattr(self, '_left_hint_surface'):
            self._left_hint_surface = self.font.render("Press 'I' for Inventory", True, (255, 255, 255))
        self.display_surface.blit(self._left_hint_surface, (10, 40))

        # Character has hp_max; max_hp is supported via alias in Character
        hp_text = f"HP: {self.player.hp}/{getattr(self.player, 'max_hp', self.player.hp_max)}"
