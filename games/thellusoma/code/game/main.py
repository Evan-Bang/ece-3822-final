...
class SaveSlotCard:
    """Visual card displaying a save slot"""
    def __init__(self, x, y, slot_number, player_name="New Game", description="", sprite_path=None):
        ...
        # Load player sprite preview
        try:
            if sprite_path:
                self.char_image = pygame.image.load(sprite_path).convert_alpha()
                self.char_image = pygame.transform.scale(self.char_image, (128, 128))
            else:
                raise FileNotFoundError
        except:
            self.char_image = pygame.Surface((128, 128))
            self.char_image.fill((150, 150, 150))
    ...
    def draw(self, surface):
        ...
        # Background
        bg_color = (100, 200, 100) if self.selected else (150, 150, 150) if self.hovered else (80, 80, 80)
        ...
        # Player name
        name_text = self.name_font.render(self.player_name, True, (255, 255, 255))
        name_rect = name_text.get_rect(center=(self.width / 2, 160))
        self.image.blit(name_text, name_rect)

        # Description
        self.draw_wrapped_text(self.description, self.desc_font, (255, 255, 255), 10, 190, self.width - 20)
...
class Game:
    ...
    def save_select(self):
        ...
        select_screen = True
        title = self.font.render("Select a Save Slot", True, (255, 255, 255))
        title_rect = title.get_rect(center=(WIDTH/2, 50))

        # Example: 3 save slots (replace with dynamic save discovery if needed)
        save_slots = [1, 2, 3]
        cards = []
        card_spacing = 220
        start_x = (WIDTH - (len(save_slots) * card_spacing - 20)) / 2

        for i, slot in enumerate(save_slots):
            data = GAMEDATA["SAVE"][str(slot)]
            if data:
                player_name = data["GAME_STATE"]["PLAYER_NAME"]
                description = data["GAME_STATE"]["PLAYER_DESCRIPTION"]
                sprite_path = data["GAME_STATE"]["PLAYER_SPRITE"]
            else:
                player_name = "New Game"
                description = ""
                sprite_path = None

            card = SaveSlotCard(start_x + i * card_spacing, 120, slot, player_name, description, sprite_path)
            cards.append(card)

        # Confirm / Back buttons
        button_width, button_height = 300, 50
        confirm_button_rect = pygame.Rect(WIDTH/2 - button_width/2, 500, button_width, button_height)
        back_button_rect = pygame.Rect(WIDTH/2 - button_width/2, 570, button_width, button_height)
        ...
        while select_screen:
            ...
            for card in cards:
                if card.is_hovered(mouse_pos):
                    card.hovered = True
                    if mouse_pressed[0] and not clicked:
                        for c in cards:
                            c.selected = False
                        card.selected = True
                        selected_card = card

            clicked = mouse_pressed[0]

            # Confirm selection
            if selected_card and confirm_button_rect.collidepoint(mouse_pos) and mouse_pressed[0]:
                self.selected_save = selected_card.slot_number
                select_screen = False
                return

            # Back button
            if back_button_rect.collidepoint(mouse_pos) and mouse_pressed[0]:
                select_screen = False
                self.intro_screen()
                return
            ...
            # Buttons
            confirm_color = (50, 200, 50) if selected_card else (100, 100, 100)
            pygame.draw.rect(self.screen, confirm_color, confirm_button_rect)
            pygame.draw.rect(self.screen, (255, 255, 255), confirm_button_rect, 2)
            confirm_text = self.button_font.render("Confirm Selection", True, (255, 255, 255))
            self.screen.blit(confirm_text, confirm_text.get_rect(center=confirm_button_rect.center))

            pygame.draw.rect(self.screen, (150, 50, 50), back_button_rect)
            ...
            self.clock.tick(FPS)
    def start_game(self):
        """Initialize the game level with selected character"""
        if self.selected_save is not None:
            self.level = Level(
                player_name=self.player_name,
                save_slot=self.selected_save,
            ...
            print("\n" + "="*50)
            print(f"GAME STARTED - Save Slot {self.selected_save}")
            print("="*50 + "\n")
            print("\n" + "="*50)
    ...
    def run(self):
        """Main game loop"""
        # Save slot selection (character select not used in this flow)
        self.save_select()
        if not self.running or self.selected_save is None:
            return

        # Initialize level via start_game() helper
        self.start_game()
        if not self.running or self.level is None:
            return
