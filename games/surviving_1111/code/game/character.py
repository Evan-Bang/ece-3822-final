...
class Character(pygame.sprite.Sprite):
    ...
    def apply_buff(self,* , stat_name, amount, duration_in_frames):
        """Apply a buff or debuff to the character"""
        buff = Buff(stat_name, amount, duration_in_frames)
        self.active_buffs.append(buff)
    
    def get_stat_with_buff(self, stat_name):
        """Get current value of a stat, considering buffs/debuffs"""
        if hasattr(self, stat_name):
            base_value = getattr(self, stat_name)
        elif hasattr(self, f"_{stat_name}"):
            base_value = getattr(self, f"_{stat_name}")
        else:
            # Unknown stat: treat as 0
            base_value = 0
        ...
        for buff in self.active_buffs:
            if buff.stat_name == stat_name:
                total_modifier += buff.amount
        if base_value + total_modifier < 0:
            return 0
    ...
    def animate(self):
        ...
        if not self.is_local:
            ...
            # Flicker while invulnerable after taking a hit
