...
class PatrolPath:
    ...
    def add_waypoint(self, x, y, wait_time=0):
        ...
        if self.head is None:
            ...
            self.tail = new_waypoint
            # For "circular", close the loop (tail.next = head, head.prev = tail)
    ...
    def get_next_waypoint(self):
        ...
        # reversing direction when hitting the end or start
            ...
            if self.direction == 1: # When moving forward
                if self.current.next is not None: # continue until the end
                    self.current = self.current.next
                else: # reverse direction at the end
                    self.direction = -1
                    self.current = self.current.prev
            elif self.direction == -1: # when moving backwards
                if self.current.prev is not None: #continue until the start
                    self.current = self.current.prev
                else: # patrol is complete after getting back to the start, reset direction and current
                    self.direction = 1
                    self.current = None
        return result
