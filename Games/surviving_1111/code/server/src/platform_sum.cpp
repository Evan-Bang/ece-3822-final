...
void report_to_python(const std::string& name, int score, long playtime) {
    ...
    // Using the key from your port_assignments.txt
    int port = get_port_from_config("PLATFORM_JSON_PORT");
    ...
    // 4. Format as a JSON string
    // We add the "type" key so the Python process_message() knows what to do
    ...
    std::cout << "[REPORTER] JSON Summary sent for " << name << " to port " << port << "\n";
