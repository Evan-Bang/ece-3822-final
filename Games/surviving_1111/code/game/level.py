...
class Level:
    ...
    def create_map(self):
        ...
        """
        # Tileset for grass layer: graphics/tilemap/ground.png
        # Each tile_id in the CSV maps to a TILESIZE×TILESIZE sub-image.
        grass_tiles = self._load_tileset('../../graphics/tilemap/ground.png')

        # --- Layer definitions ---
        # (csv_path, sprite_type, groups, tileset_dict)
        LAYERS = [
            ('map/map_FloorBlocks.csv', 'boundary',
             [self.visible_sprites, self.obstacle_sprites, self.visible_sprites], grass_tiles),
            ('map/map_Grass.csv',       'grass',
             [self.visible_sprites],                        grass_tiles),
            ('map/map_Objects.csv',     'object',
             [self.visible_sprites, self.obstacle_sprites], grass_tiles),
        ]
        ...
        for csv_path, sprite_type, groups, tileset in LAYERS:
            try:
                ...
                for (row, col), tile_id in entries:
                    x = col * TILESIZE
                    y = row * TILESIZE
                    surf = tileset.get(tile_id)   # None → Tile uses its default surface
                    if surf is not None:
...
class YSortCameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self, player):
        """Draw sprites sorted by layer then Y position"""
        self.offset.x = player.rect.centerx - self.half_width
        self.offset.y = player.rect.centery - self.half_height

        def draw_layer(sprite):
            sprite_type = getattr(sprite, 'sprite_type', None)
            if sprite_type == 'grass':
                ...
                layer = 3  # players and enemies
            return (layer, sprite.rect.centery)

        for sprite in sorted(self.sprites(), key=draw_layer):
