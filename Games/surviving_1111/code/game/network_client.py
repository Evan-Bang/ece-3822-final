...
class NetworkClient:
    ...
    def _process_message(self, msg):
        ...
        if msg.startswith("CONNECTED|"):
            ...
            for i in range(1, len(parts)):
                if parts[i]:
                    #print(f"[DEBUG] Parsing player {i}: '{parts[i][:50]}...'")
            ...
            _, name, text = msg.split("|", 2)
            self._pending_chat.append({'name': name, 'text': text})
    ...
    def _deserialize_binary(self, data):
        ...
        try:
            ...
            x = unpacked[2]
            y = unpacked[3]
            character_type = unpacked[5].decode('utf-8').rstrip('\x00')
    ...
    def send_chat(self, text: str):
        ...
        if not self.connected:
            return
        clean = text.replace("|", "")  # strip pipes so delimiter is safe
        msg = f"CHAT|{self.player_name}|{clean}\n"        
        try:
            self.sock.send(msg.encode('utf-8'))
        except:
            self.connected = False
    ...
    def get_chat_messages(self):
        """Return and clear any chat messages received since last call"""
        msgs = self._pending_chat
        self._pending_chat = []
        return msgs
